...
METHOD_ROOT="${RELEASE_ROOT}/mph-gait"
CODE_ROOT="${METHOD_ROOT}/code"
CONFIG=${CONFIG:-${FINAL24_ROOT}/configs/mph_gait_final24_len15.yaml}
PYTHON_CMD=${PYTHON_CMD:-"conda run --no-capture-output -n pytorch python"}
SEEDS=${SEEDS:-"0 1 2"}
RUN_STAMP=${RUN_STAMP:-$(date +%Y%m%d-%H%M%S)}
OUTPUT_BASE=${OUTPUT_BASE:-${RELEASE_ROOT}/outputs/final-training/mph-gait}
...
for SEED in ${SEEDS}; do
  ...
  if [[ "${SKIP_COMPLETED}" == "1" && -f "${SEED_ROOT}/retrieval_metrics.json" ]]; then
    printf '[skip completed] MPH-Gait seed=%s\n' "${SEED}"
    continue
  fi
  printf '\n[MPH-Gait Final-24] seed=%s output=%s\n' "${SEED}" "${SEED_ROOT}"
  TRAIN_ARGS=(
    -m mph_gait.train
    --config "${CONFIG}"
    --seed "${SEED}"
