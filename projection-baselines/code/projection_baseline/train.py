...
from projection_baseline.dataset import (
    ProjectedGaitProtocolDataset,
    canonical_split,
)
from projection_baseline.model import build_model, part_cross_entropy
from gait_core.protocol import build_fixed_special5_protocol
...
_DATASET_TEMPLATE_CACHE: dict[tuple[Any, ...], ProjectedGaitProtocolDataset] = {}
...
def build_dataset(
    ...
    cache_key = (
        str(Path(data_cfg.get("root", "dataset_proj")).resolve()),
        str(data_cfg.get("dataset_name", "PersonRecognitionWalking_29")),
        str(data_cfg.get("mode", "lidargait_rgb")),
    ...
    if template is None:
        template = ProjectedGaitProtocolDataset(
            root=data_cfg.get("root", "dataset_proj"),
            split="all",
            mode=data_cfg.get("mode", "lidargait_rgb"),
...
def classification_loss(
    ...
    scale = float(cfg.get("loss", {}).get("ce_scale", 16.0))
    return part_cross_entropy(outputs["part_logits"], labels, scale=scale)
...
def save_provenance(run_dir: Path, config_path: Path) -> None:
    source_paths = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train projection baselines under the fixed-special5 protocol."
    )
    parser.add_argument(
        "--config",
        default="projection_baseline/configs/rgb_depth_len15.yaml",
    )
...
def main() -> None:
    ...
    experiment_root = str(cfg.get("experiment", {}).get("root", ""))
    if not experiment_root:
        raise ValueError("experiment.root must be configured")
    output_root = (ROOT.parent / "outputs").resolve()
    try:
        Path(experiment_root).resolve().relative_to(output_root)
    except ValueError as error:
        raise ValueError(
            "Projection outputs must stay under "
            f"{output_root}, got: {experiment_root!r}"
        ) from error
    ...
    protocol = build_fixed_special5_protocol(cfg, available_ids)
    ...
    save_provenance(run_dir, config_path)
    ...
    if len(train_set) == 0 or len(val_set) == 0:
        raise RuntimeError(
            f"Empty train/validation dataset: train={len(train_set)}, val={len(val_set)}"
        )
    ...
    print(
        f"protocol: {protocol['name']} split={split_index}/"
        f"{protocol['num_splits']}"
    )
    ...
    model_cfg["in_channels"] = int(
        data_cfg.get("channels", model_cfg.get("in_channels", 3))
    )
    ...
    if args.dry_run:
        ...
        print(f"dry_run images: {tuple(images.shape)}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        retrieval_metrics: dict[str, Any] = {}
        if retrieval_every > 0 and epoch % retrieval_every == 0:
        ...
        selected = selection_value(retrieval_metrics)
    ...
    if not best_path.exists():
        raise RuntimeError("No best checkpoint was selected from validation mAP")
